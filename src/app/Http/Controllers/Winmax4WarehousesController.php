...
{
    /**
     * @var Winmax4WarehouseService The service responsible for handling interactions with the Winmax4 API.
     */
    protected $winmax4Service;

    /**
     * Init Constructor for Winmax4WarehousesController.
     *
     * This constructor initializes the `Winmax4WarehouseService` based on the settings retrieved from the database.
    ...
    public function __construct()
    {
        ...
        if(!$winmaxSettings) {
            $this->winmax4Service = new Winmax4WarehouseService(true);
        }else{
            $this->winmax4Service = new Winmax4WarehouseService(
                false,
                $winmaxSettings->url,
     ...
     *
     * This method queries the Winmax4Warehouse model using Eloquent ORM to fetch
     ...
     *
     * ```php
     * $response = $this->getWarehouses();
     * ```
     *
     * ### Response Format
     *
     * The response is a JSON-encoded array of objects, each representing a record from the
     * Winmax4Warehouse model. The format of each object corresponds to the columns of the
